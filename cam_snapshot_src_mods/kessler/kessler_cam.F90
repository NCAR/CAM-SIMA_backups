...
!========================================================================================
  ...
  subroutine kessler_tend(state, ptend, ztodt, pbuf)
    ...
    type(physics_state), intent(inout) :: state !DEBUG -JN
    ...
    !Debug -JN:
    !write(*,*) 'DEBUG -JN timestep: ', get_nstep()
    !write(*,*) 'DEBUG -JN: water vapor pre-calc', qv(107,10), get_nstep(), get_lat_p(1,107), get_lon_p(1,107)
    !write(*,'(a,ES46.40)') 'DEBUG -JN: cloud water pre-calc: ', qc(134,11)

    !Debug -JN:
    !write(*,'(a,ES46.40)') 'DEBUG -JN: cpairv pre-exner: ', cpairv(50,10,lchnk)
    !write(*,'(a,ES46.40)') 'DEBUG -JN: cpdair pre-exner: ', shr_const_cpdair
    !write(*,'(a,ES46.40)') 'DEBUG -JN: rairv pre-exner: ', rairv(50,10,lchnk)
    !write(*,'(a,ES46.40)') 'DEBUG -JN: rdair pre-exner: ', shr_const_rdair
    !write(*,'(a,ES46.40)') 'DEBUG -JN: psurf_ref pre-exner: ', psurf_ref
    !write(*,'(a,ES46.40)') 'DEBUG -JN: pmid pre-exner: ', state%pmid(50,10)
    ...
    !DEBUG -JN:
    !write(*,'(a,ES46.40)') 'DEBUG -JN: exner-func post-exner: ', pk(50,10)
    ...
    !Debug -JN:
    !write(*,'(a,ES23.17,ES23.17)') 'DEBUG -JN: water vapor dry pre-calc: ', qv(50,10), qv_dry(50,10)
    !write(*,'(a,ES46.40)') 'DEBUG -JN: cloud water dry pre-calc: ', qc_dry(134,11)
    ...
    !Debug -JN:
    !write(*,'(a,ES23.17)') 'DEBUG -JN: water vapor dry post-calc: ', qv_dry(50,10)
    !write(*,'(a,ES23.17)') 'DEBUG -JN: water vapor dry post-calc: ', qc_dry(50,10)
    !write(*,'(a,ES46.40)') 'DEBUG -JN: cloud water dry post-calc: ', qc_dry(134,11)
    ...
    !Debug -JN:
    !write(*,'(a,ES46.40)') 'DEBUG -JN: cloud water post-calc: ', qc(134,11)

    ! Back out tendencies from updated fields
    do k = 1, pver
       !DEBUG -JN:  Convert enthalpy tendency to just a temperature tencency:
       !ptend%s(:ncol,k)          = (th(:ncol,k)*pk(:ncol,k) - state%t(:ncol,k)) * cpairv(:ncol,k,lchnk) / ztodt
       ptend%s(:ncol,k)          = (temp(:ncol,k) - state%t(:ncol,k)) / ztodt
       ptend%q(:ncol,k,1)        = (qv(:ncol,k) - state%q(:ncol,k,1)) / ztodt
       !ptend%q(:ncol,k,ixcldliq) = (qc(:ncol,k) - state%q(:ncol,k,ixcldliq)) / ztodt
       !DEBUG -JN: Set state to qc:
       !--------------------------
       state%q(:ncol,k,ixcldliq) = qc(:ncol,k)
       ptend%q(:ncol,k,ixcldliq) = 0._r8
       !--------------------------
