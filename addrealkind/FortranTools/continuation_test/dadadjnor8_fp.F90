...
!
   ...
   real(fp), intent(in) :: pmid(pcols,pver)   ! pressure at model levels
   real(fp), intent(in) :: pint(pcols,pverp)  ! pressure at model interfaces
   real(fp), intent(in) :: pdel(pcols,pver)   ! vertical delta-p

!
! Input/output arguments
!
   real(fp), intent(inout) :: t(pcols,pver)      ! temperature (K)
   real(fp), intent(inout) :: q(pcols,pver)      ! specific humidity
!
!---------------------------Local workspace-----------------------------
!
   ...
   real(fp) c1dad(pver)        ! intermediate constant
   real(fp) c2dad(pver)        ! intermediate constant
   real(fp) c3dad(pver)        ! intermediate constant
   real(fp) c4dad(pver)        ! intermediate constant
   real(fp) gammad             ! dry adiabatic lapse rate (deg/Pa)
   real(fp) zeps               ! convergence criterion (deg/Pa)
   real(fp) rdenom             ! reciprocal of denominator of expression
   real(fp) dtdp               ! delta-t/delta-p
   real(fp) zepsdp             ! zeps*delta-p
   real(fp) zgamma             ! intermediate constant
   real(fp) qave               ! mean q between levels
...
!-----------------------------------------------------------------------
!
   zeps = 2.0e-5_fp           ! set convergence criteria
!
! Find gridpoints with unstable stratification
!
   do i=1,ncol
      gammad = cappa*0.5_fp*(t(i,2) + t(i,1))/pint(i,2)
      dtdp = (t(i,2) - t(i,1))/(pmid(i,2) - pmid(i,1))
      dodad(i) = (dtdp + zeps) .gt. gammad
   end do
   do k=2,nlvdry
      do i=1,ncol
         gammad = cappa*0.5_fp*(t(i,k+1) + t(i,k))/pint(i,k+1)
...
!
   do 80 i=1,ncol
      if (dodad(i)) then
         zeps = 2.0e-5_fp
         do k=1,nlvdry
            c1dad(k) = cappa*0.5_fp*(pmid(i,k+1)-pmid(i,k))/pint(i,k+1)
            c2dad(k) = (1._fp - c1dad(k))/(1._fp + c1dad(k))
            rdenom = 1._fp/(pdel(i,k)*c2dad(k) + pdel(i,k+1))
