...
#TBH:  Note that the following is NOT IMPLEMENTED YET.  See "^#TBH" for 
#TBH:  commented-out code that implements this, if needed.  
#TBH   Masking out an entire line that looks like a cpp directive.  A line that 
#TBH   begins with a "#" is assumed to be a cpp directive.  Note that the 
#TBH   "continued" state of a cpp directive is not known by this class (because 
#TBH   an #include directive may or may not insert code that ends in a continued 
#TBH   line).  This must be deduced by users of this class.  
#==============================================================================
class FortranLine

  attr_reader :stringRegions, :nextStringStart, :lineNumber, 
    :string_delimiter, :orig_string_delimiter, :commentLine,
    :commentIndex, :contEndIndex, :contBeginIndex, :maskedLine
  attr_accessor :line, :continued, :prev_was_contd
  ...
  def initialize(*args)
    if (args.length <= 4) then
      ...
      # is this a comment line? (optimization)
      @commentLine = false
...
#$$$ replace this mess with a Hash!  
    ret = FortranLine.new(copy_stringRegions, copy_nextStringStart, 
                          ...
                          copy_contBeginIndex, copy_maskedLine, copy_commentLine)
  end
...
#$$$ put this mess in an Array
    ...
    # do not need to check @commentLine because it is derived
...
#$$$ eventually replace this with functionality from FortranStatement!
  def line_stripped
...
#TBH If a line begins with a "#" it is assumed to be a cpp directive and 
#TBH the returned Range will mask out the entire line.  
...
#TBH cpp directive
#TBH "continued" state is unknown (because an #include directive may or may 
#TBH not insert code that ends in a continued line).  This must be deduced 
#TBH by users of this class.  
#TBH    if (@line =~ /^#/) then
#TBH      ret << (0..(@line.length - 1))
#TBH    end
    ret
  end
