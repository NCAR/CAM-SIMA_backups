...
#   cpp directives embedded in Fortran statements do not contain unbalanced 
#   parenthesis.  
#
# Limitations:  
#   Understanding of cpp directives is not complete.  cpp directives embedded 
#   in Fortran character literals will not be processed correctly.  Embedded 
#   #include directives that point to files that contain Fortran source code 
#   that has unbalanced parenthesis will be identified as erroneous.  For 
#   example, the following cases are OK:  
...
#                     )
#
#   However, the following case is identified as erroneous (this class will 
#   not deduce that the missing right-parenthesis must be inside the #include 
#   file):  
#         SUBROUTINE X(arg1, arg2, &
#   #include 'subxargs'
#
#   The following case is also identified as erroneous for the same reason:  
#   #if ( NMM_CORE == 1 )
#              CALL myjsfcinit(LOWLYR,UST,Z0,XLAND,XICE,           &
#   #else
...
#                 supported yet.  Some work-arounds for cpp directives.  
#
#==============================================================================
...
class FortranStatement
  ...
  attr_reader :format, :cpp_directive, :contains_cpp_directives
...
#$$$here...  why not just leave the "\n"s in to begin with and avoid all this crap?  
  def get_statement(lines)
    ...
    until (done) do
      ...
      # @cpp_directive==true iff statement is a single cpp directive
      @cpp_directive = false
      # @contains_cpp_directives==true iff statement has one or more continued 
      # lines that are cpp directive(s)
      @contains_cpp_directives = false
      if (thisLine.line =~ /^#/) then
        if (firstLine) then
          @cpp_directive = true
        else
          @contains_cpp_directives = true
        end
      end
      done = true
      # CR_HACK, see CR_HACK below
      embedded_cpp_begin_cr = ""
      # if this line is continued, we're not done yet
      if (thisLine.is_continued?) then
        done = false
      # if this line is a cpp directive embedded in a Fortran statement and 
      # parenthesis do not balance, assume that the line is continued and 
      # keep going
      elsif ((thisLine.line =~ /^#/) and (not firstLine)) then
        lparens = 0
        ret.each_unmasked do |str|
          lparens = lparens + str.count("(")
        end
        rparens = 0
        ret.each_unmasked do |str|
          rparens = rparens + str.count(")")
        end
        unless (lparens == rparens) then
          # continue
          done = false
          # finally, propagate "continued" state across this line and any 
          # following comment lines or cpp directives (ugh!)
          thisLine.continued = true
          lines.each do |line|
            if ((line.commentLine) or (line.line =~ /^#/)) then
              line.prev_was_contd = true
              line.continued = true
            else
              line.prev_was_contd = true
              break
      ...
      # finally, append thisLine to this statement
      # this is done here so parenthesis checking (above) is done before 
      # thisLine is appended
      @myLines << thisLine
      stripped = thisLine.line_stripped
      # CR_HACK for special printing of cpp directives that are embedded within 
      # Fortran statements
      embedded_cpp_cr = ""
      if (@contains_cpp_directives and (thisLine.line =~ /^#/)) then
        embedded_cpp_cr = "\n"
      end
      @statement << embedded_cpp_begin_cr + stripped + embedded_cpp_cr
  ...
  # Note that classifications @cpp_directive and @contains_cpp_directives 
  # are both set in get_statement().  
  # (incomplete!)
#$$$ need to search @collapsed.get_unmasked instead of @statement !!  
#$$$ refactor classifications into a separate class!
  def classify
    @module_begin = false
    @module_end = false
    @module_name = nil
...
#  end
  ...
  # findSymbols(aString, [true | false], [true | false])
  # Find all symbols matching the first argument.  Returns an Array of String.  
  # If second argument is true, then only symbols appearing in 
  # PUBLIC statements or whose declarations include the PUBLIC attribute 
  # will be included.  Duplicates are removed.  
  # If third argument is true, then Fortran keywords "operator" and 
  # "assignment" are also included along with the following pair of 
  # parenthesis.  
  # Fortran character literals, FORMAT statements, cpp directives, 
  # and comments are avoided.  
#$$$ first arg should really be a Regexp !!  
  def findSymbols(symbol, public, operator)
#print "DEBUG FoundStatement::findSymbol():  operator = #{operator}\n"
    retall = []
    if (@verbose) then
      print "#{self.class}.findSymbols:  searching str = <<#{@statement}>>\n"
    end
    unless (@format or @cpp_directive) then
      searchstr = @collapsed.get_unmasked
#print "DEBUG:  searching str = <<#{searchstr}>>...\n"
      if ((not public) or (searchstr =~ /public/i)) then
        offset = 0
        if (location = (searchstr =~ /::/)) then
          offset = location + 2
        end
        #$$$ this is a HACK!!!  FIX IT!!!  
        fakestr = "X(" + searchstr[(offset..(searchstr.length-1))] + ")"
#print "DEBUG:  fakestr = <<#{fakestr}>>\n"
        ap = ArgumentParser.new(fakestr)
        candidates = ap.get_args
        candidates.each do |sym|
          if (sym =~ /\w*#{symbol}\w*/i) then
            retall << $&.dup
          end
          if (operator) then
            # special optional treatment of Fortran keywords "operator" and 
            # "assignment"
            if (sym =~ /(^|\W+)(assignment|operator)(\s*)\(/i) then
              m2 = $2.dup
              m3 = $3.dup
              post_match = $'.dup
              if (post_match =~ /^.+\)/) then
                ...
                raise "ERROR:  missing close-parenthesis in #{m2} statement.\n"
              end
            end
          end
        end
  ...
  # replaceSymbols(aString, aString)
  # Find all symbols matching the first argument and replace them with the 
  # second argument.  Returns aString that may contain one or more "\n".  
  # Fortran character literals, FORMAT statements, cpp directives, 
  # and comments are avoided.  
#$$$ remove duplication with addRealKind!  
#$$$ first arg should really be a Regexp !!  
  def replaceSymbols(symbol, replacement)
#print "DEBUG FoundStatement::replaceSymbol():  operator = #{operator}\n"
    retall = nil
    unless (@format or @cpp_directive) then
      newcollapsed = @collapsed.deepCopy
      newcollapsed.modify_all do |str|
        if (@verbose) then
          print "#{self.class}.replaceSymbols:  replacing <<#{symbol}>> with <<#{replacement}>> in <<#{str}>>\n"
        end
        str.gsub!(/(\w*)(#{symbol})(\w*)/i) do
          pre = $1.dup
          post = $3.dup
          ret = "#{pre}#{replacement}#{post}"
        end
      end
      retall = newcollapsed.baseString
      if (@verbose) then
        print "#{self.class}.replaceSymbols:  returning str = <<#{retall}>>\n"
      end
    else
  ...
  def addRealKind(*args)
    ...
    unless (@format or @cpp_directive) then
      ...
      newcollapsed.modify_all do |str|
        ...
        str.gsub!(/REAL|COMPLEX/i) do
          ...
          if (add_kind) then
            $found_missing=1
            if (@log_only) then
              print $fileName,":",$myindx,@collapsed.baseString
            else
              ret = "#{match}(#{realKind})"
            end
  	  end
 	...
 	if (@verbose) then
      ...
      testvar=@collapsed.baseString.match("[!&]").to_s
      if (testvar !~ /!/) then
        $myindx = $myindx + @collapsed.baseString.scan("\n").size-1
      end
      retall = newcollapsed.baseString
      if (@verbose) then
...
class FortranStatements
  ...
  def initialize(*args)
    ...
    tmpFortranLineReader = FortranLineReader.new($fileName,@verbose,@log_only)
    until (tmpFortranLineReader.empty?) do
      @statements << FortranStatement.new(tmpFortranLineReader,@verbose,@log_only)
    end
  end
...
#print"DEBUG:  found end of module <<#{modname}>> at line #{indx}\n"
  ...
  # parenthesis.  
  # Fortran character literals, FORMAT statements, and comments are avoided.  
  # All symbols are converted to uppercase and then 
  # duplicates are removed (this is Fortran...).  
#$$$ first arg should really be a Regexp !!  
  def findSymbols(symbol, public, operator)
#print "DEBUG FoundStatements::findSymbol():  operator = #{operator}\n"
    ret = []
    @statements.each do |statement|
      if (@verbose) then
      ...
      begin
        ret = ret + statement.findSymbols(symbol, public, operator)
      rescue
  ...
  # replaceSymbols(aString, aString)
  # Find all symbols matching the first argument and replace them with the 
  # second argument.  Returns anArray of aString.  
  # Fortran character literals, FORMAT statements, and comments are avoided.  
#$$$ first arg should really be a Regexp !!  
  def replaceSymbols(symbol, replacement)
    ret = []
    @statements.each do |statement|
      if (@verbose) then
        print "#{self.class}.replaceSymbols:  file #{@fileName}, line #{statement.firstLineNumber}, statement = <<#{statement}>>\n"
      end
      begin
        ret << statement.replaceSymbols(symbol, replacement)
      rescue
        print "ERROR replacing in file #{@fileName} at line #{statement.firstLineNumber}\n"
        raise
      end
    end
    ret
