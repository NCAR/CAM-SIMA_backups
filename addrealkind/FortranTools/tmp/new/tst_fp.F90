...
!-----------------------------------------------------------------------
   ...
   real(r8) :: need_a_kind, xfp, blahr, a3d0var, a3e0var, a3d0, a3e0
   COMPLEX(r8) :: calready_got_kind, cxr8
   complex(r8) :: cneed_a_kind, cxfp
   ...
   real(r8)::realvar                            ! add kind

   ! combination of REAL declaration and REAL intrinsic
   REAL(r8), PARAMETER :: r1 = REAL(1,r8)          ! need two kinds
   ...
   xfp = 1.0_r8       ! need a kind
   xfp = .2_r8        ! need a kind
   xfp = 3._r8        ! need a kind
   xfp = 4E0_r8       ! need a kind
   xfp = 5E-1_r8      ! need a kind
   xfp = 6.0E+2_r8    ! need a kind
   xfp = .7E-3_r8     ! need a kind
   xfp = 8.E+4_r8     ! need a kind
   xfp = 9.0E5_r8     ! need a kind
   xfp = .10E6_r8     ! need a kind
   xfp = 11.E7_r8     ! need a kind
   xfp = -1.0_r8      ! need a kind
   xfp = +.2_r8       ! need a kind
   xfp = -3._r8       ! need a kind
   xfp = +4e0_r8      ! need a kind
 ...
 real: IF ( xfp.lt.1 ) THEN  ! leave as-is
   ...
   ENDIF real
   IF ( xfp.lt.1._r8 ) THEN  ! need a kind
     print *,'xfp.lt.1.'  ! leave as-is
   ENDIF
...
CMPLX: IF ( 1.lt.xfp ) THEN  ! leave as-is
     print *,'1.lt.xfp'  ! leave as-is
   ENDIF CMPLX
   IF ( 1._r8.lt.xfp ) THEN  ! need a kind
     print *,'1..lt.xfp.' ! leave as-is
   ENDIF

   blahr = REAL(blahd,r8) ! add kind as second argument to intrinsic cast function
   blahr = REAL( &
                 blahd,r8) ! add kind as second argument to intrinsic cast function
   blahr = REAL( &
                 blahd, &
                 fp) ! leave as-is
   ! add kinds as second arguments to intrinsic cast functions to some of the following
   blahr = REAL(blahd + REAL(blahd2,r8),r8)
   blahr = REAL(blahd + REAL(blahd2,r8),r8) + REAL(blahd + REAL(blahd2,r8),r8)
   blahr = 1.0_r8 + REAL(blahd + REAL(blahd2,r8),r8) + 2.0_r8
   blahr = REAL(blahd + REAL(blahd2,r8), r8)
   blahr = REAL(blahd + REAL(blahd2, r8),r8)
   blahr = REAL(blahd + REAL(blahd2, r8), r8)
   blahr = REAL(blahd + &
   ...
   blahr = REAL(blahd + &              ! this is just plain mean
             REAL(blahd2,r8),r8) + &
               REAL(blahd + REAL(&
                 blahd2,r8) &
                 ,r8)
   blahr = REAL(REAL(blahd + REAL(blahd2,r8),r8) + &       ! OK enough already
             REAL(INT(REAL(blahd + REAL(blahd2,r8),r8)),r8),r8)
