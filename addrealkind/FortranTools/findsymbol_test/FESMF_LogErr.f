...
!EOP
    ...
    if (statusToCheck .NE. 0) then
        if (present(rcToReturn)) rcToReturn=30
        call c_esmc_loggeterrormsg(30,tempmsg,msglen)
...
!EOPI
    ...
    if (present(lognone)) then
      if (lognone .eq. 0) ESMF_LogDefault%logNone = ESMF_TRUE 
    endif
...
!EOP
    ...
    if (present(lognone)) then
      if (lognone .eq. 0) log%logNone = ESMF_TRUE 
    endif
    if (present(logtype)) then
      	log%logtype=logtype
    else
