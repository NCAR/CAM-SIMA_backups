...
class TestFortranStatements
  ...
  def initialize(infname, outfname, outfname_OK, unmaskfname, unmaskfname_OK, 
                 ...
                 symrepl_fname, symreplfname_OK, replacement_in)
    @infname = infname
    @outfname = outfname
    ...
    @symreplfname = symrepl_fname
    @symreplfnameOK = symreplfname_OK
    @replacement = replacement_in
  ...
  def findsymrepl_correct?
    ret = true
    if (@replacement) then
      File.open(@symreplfname, "w") do |aFile|
        aFile.puts(@statements.replaceSymbols(@symbol,@replacement))
      end   # File.open
      repl_OK = IO.readlines(@symreplfnameOK)
      repl    = IO.readlines(@symreplfname)
      ret = resultOK?(repl_OK, repl, "replace symbols")
      unless (ret) then
        `xxdiff #{@symreplfnameOK} #{@symreplfname}`
      end
    end
    ret
  end
...
class TC_FortranStatements < Test::Unit::TestCase
  ...
  def setup
    ...
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/c_fpuse_OK.f90", nil, nil, nil, nil, nil, nil, nil) 
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/tst0_fpuse_OK.F90", 
    ...
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/tst1_repl.F90", 
                      "continuation_test/tst1_repl_OK.F90", 
                      "sHAm" )
    @statementss << TestFortranStatements.new(
                      ...
                      [],
                      [], 
                      "continuation_test/tst_fp_repl.F90", 
                      "continuation_test/tst_fp_repl_OK.F90", 
                      "WRF_COMP_" ) 
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/dadadj_fpuse_OK.F90", 
                      nil, nil, nil, nil, nil, nil, nil) 
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/dadadjnor8_fpuse_OK.F90", 
                      nil, nil, nil, nil, nil, nil, nil) 
  end
  ...
  def test_symreplace
    @statementss.each do |statements|
      assert(statements.findsymrepl_correct?, statements.errorMsg)
    end
  end
