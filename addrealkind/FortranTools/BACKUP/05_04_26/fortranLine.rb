...
#$$$ put this mess in an Array
  ...
  # get masked line
#$$$ get rid of this after fixing depedencies in callers
  def get_masked_line
    @maskedLine
  end
...
#$$$  all real literal flavors
#$$$  existing real(r8), real(fp), (etc.) declarations
#$$$  existing _r8, _fp, (etc.) literal constants
#$$$  complex
#$$$  real() intrinsic function
#$$$  format statements
#$$$  all character case combinations
  def addRealKind(realKind, skipLine)
#$$$ REPLACE this inefficient implementation with a bit of syntax-aware code 
#$$$ that identifies and masks everything but real declarations and literals 
#$$$ so we can just do a few substitutions.  
    unless (skipLine) then
      self.each_unmasked_op! do |substr| 
        ...
        substr.gsub!(/(REAL|COMPLEX)\(#{realKind}\)(\s*\()/i) do 
          "#{$1}#{$2}"
        end
        # handle REAL and COMPLEX literal constants
#$$$here...  Hooray, this works!  Add remaining regexp bits and skip FORMAT lines...  
        substr.gsub!(/(\d+\.\d*)/) do 
          "#{$1}_#{realKind}"
        end
      end
