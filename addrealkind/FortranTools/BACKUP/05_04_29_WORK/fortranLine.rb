...
#$$$ put this mess in an Array
  ...
  # Perform operations in block on each unmasked substring.  
  # Operations on each unmasked substring are performed in isolation.  
  # Modifies self in-place.  If self changes, then all masked and 
  # unmasked ranges are re-computed to account for operations that 
  # change substring length or add Fortran comments or Fortran strings.  
  # Supplied block must use mutator variants to modify the String argument 
  # in-place.  For example, the following is OK:  
  #    each_unmasked_op! { |str| str.gsub!(...) }   # OK, mutator used
  # while the following will *NOT* change self at all:  
  #    each_unmasked_op! { |str| str.gsub(...) }    # BAD, mutator NOT used
  # NOTE:  when using regexp variables like "$1", "$2" (etc.) the block form 
  # of substitution operators *MUST BE USED* due to block scoping issues.  
  # For example, the following works:  
  #   self.each_unmasked_op! do |substr|
  #     substr.gsub!(/(REAL)/i) do
  #       "#{$1}(#{realKind})"
  #     end
  #   end
  # However, this does *NOT* work since "$1"=="" in the calling scope:  
  #   self.each_unmasked_op! do |substr|
  #     substr.gsub!(/(REAL)/i, "#{$1}(#{realKind})")
  #   end
  def each_unmasked_op!(&block)
    changed = @maskedLine.each_unmasked(&block)
    if (changed) then
      @line = @maskedLine.baseString
      # re-compute masked areas just in case the block added 
      # comments or Fortran strings.  
      #$$$ optimize later to avoid this if nothing changed via return value from each_unmasked
      @string_delimiter = @orig_string_delimiter
      self.init_finish
    end
  end
  ...
  # Note:  abuse of whitespace (like "r e a l") is not yet supported!
  # Argument skipLine allows the caller to suppress substitutions for 
  # FORMAT statements, (etc.).  
  # Argument firstRealIsNotDecl indicates that the first occurrence of 
  # "REAL" in this line is not a REAL() declaration.  It is used to 
  # identify calls to the REAL() intrinsic function that occur at the 
  # beginning of a continued line, such as:  
  #   x = &
  #     REAL(y)
  # but NOT:  
  #   x = REAL & 
  #       (y)
###$$$here...  implement "REAL(y)" -> "REAL(y,fp)" and add test cases
###  def addRealKind(realKind, skipLine, firstRealIsNotDecl)
###$$$  pass in a single object that can be queried for skipLine, firstRealIsNotDecl, etc.  
###$$$  mask out declarations on-the-fly?  
  def addRealKind(realKind, skipLine)
    unless (skipLine) then
      self.each_unmasked_op! do |substr| 
        # hide "dot" operators
        substr.gsub!(/\.(eq|ne|lt|le|gt|ge|and|or|eqv|neqv|not)\./i) do 
          match = $1.dup
          ret = "{{DOT}}#{match}{{DOT}}"
        end
        # Handle REAL and COMPLEX casts via intrinsic function
        substr.gsub!(/(REAL|CMPLX)(\s*)\((.+)\)/i) do 
          match = $&.dup
          pre_match = $`.dup
          post_match = $'.dup
          type_str = $1.dup
...
print "DEBUG:  ret = <<#{ret}>>\n"
        ...
        substr.gsub!(/(((\d+\.\d*)|(\d*\.\d+))(((e|d)(-|\+*)(\d+))|\d*))|(\d+(e|d)(-|\+*)\d+)/i) do
          match = $&.dup
          pre_match = $`.dup
          post_match = $'.dup
          ret = match
        ...
        substr.gsub!(/\{\{DOT\}\}(eq|ne|lt|le|gt|ge|and|or|eqv|neqv|not)\{\{DOT\}\}/i) do 
          match = $1.dup
          ret = ".#{match}."
        end
      end
