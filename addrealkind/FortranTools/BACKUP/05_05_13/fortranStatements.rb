...
#  end
  ...
  # findSymbols(symbol [, true | false])
  # findSymbols(aString)
  # Find all symbols matching the string.  Returns an Array of String.  
  def findSymbols(*args)
    ...
    @collapsed.each_unmask do |str|
      if (public) then
        if (str =~ /public\s+(\w*#{symbol}\w*)/i) then
          retall << $1
        end
      else
