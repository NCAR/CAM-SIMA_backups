...
class TC_FindSymbolApp < Test::Unit::TestCase
  ...
  def test_find_public_op_gen
    ...
    `#{@app} -d #{@sourcedir} -S ESMF_ -p -O -g WRF_COMP_ -o ESMF_Mod -n module_comp -w #{outfile}`
    out_OK = IO.readlines("#{outfile_OK}")
    out    = IO.readlines("#{outfile}")
    result = (out_OK == out)
    unless (result) then
    ...
    assert(result, "find public symbols and operators and generate translation module")
