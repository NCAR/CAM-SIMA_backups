  ...
  def find_close_parenthesis(open_parens_index)
    ...
    while (found > 0)
      ...
      if (offset > self.length) then
        raise "ERROR:  ran past end of string trying to find matching \")\""
      end
    end
    ret
  end
    ...
    if ((@in_str.count("(") > 0) and (@in_str.count(")") > 0) and
      ...
      if (arg_str =~ /^\s*$/) then
        ...
        if (arg_str.count("(") != arg_str.count(")")) then
          raise "ERROR:  mismatched parenthesis in argument substring"
        end
      ...
      raise "ERROR:  could not find/parse argument list"
    end
  end
