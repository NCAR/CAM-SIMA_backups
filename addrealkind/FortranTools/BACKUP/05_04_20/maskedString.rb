...
#$$$ pass in masked vs. unmasked flag?  
  def each_unmasked
    ...
    @maskUnmask.each do |murang|
      if (murang.unmasked?) then
        ...
        if (opstr != @baseString[rang]) then
          ...
          # change range
          if (opstr.length != rang.length ) then
            changed_range = true
            rangestart = rang.first
            rangeend = rangestart + opstr.length - 1
            # discard zero-length ranges
            if (rangeend >= rangestart) then
              newmaskUnmask << 
                MaskUnmaskRange.new((rangestart..rangeend),murang.mask_flag)
            end
          end
        else
          newmaskUnmask << murang
        end
      else
        newmaskUnmask << murang
      end
