...
#   possible addition of non-Fortran characters.  
#==============================================================================
class ArgumentParser
  ...
  def find_args
    if (@arg_str.count("(") != @arg_str.count(")")) then
      ...
      if (@arg_str.count("(") == 0) then
        @num_args = @arg_str.count(",") + 1
      else
        # search for outermost matching () pairs and commas
$$$here...  add code
$$$here...  put in separate file!  add require statement, add unit tests!  
      end
    end
  end
...
#  end
  ...
  # The returned String may contain more than one or more "\n".  
