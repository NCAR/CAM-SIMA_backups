...
#  end
  ...
  def addRealKind(realKind)
    ...
    unless (self.is_format?) then
      ...
      newcollapsed.modify_all do |str|
        ...
        translate_casts(realKind, str, false)
  ...
  # Flag noDeclaration is used for recursion to avoid confusing 
  # substrings with declarations.  
  # translate_casts(aString, aString, true|false)
  def translate_casts(realKind, str, noDeclaration)
#$$$here...  fix "blahr = REAL(blahd + REAL(blahd2)) + REAL(blahd + REAL(blahd2))"
...
#print "DEBUG:  pre_match = <<#{pre_match}>>\n"
      ...
      unless (noDeclaration) then
        if ( pre_match =~ /^\s*\d*\s*$/ ) then
          add_kind = false
        end
      end
      # Do not modify if a kind argument already exists.  
      if (add_kind) then
        # recursion
#$$$here...  put limit on recursion depth?  
        translate_casts(realKind, arg_str, true)
        args = ArgumentParser.new(arg_str)
        # returns nil if confused
        numargs = args.num_args
        if (numargs) then
          if (numargs != 1) then
            add_kind = false
          end
        else
          raise "ERROR:  could not parse arguments to #{type_str} intrinsic"
        end
      end
#$$$here...
#$$$here...  Add tests like:
