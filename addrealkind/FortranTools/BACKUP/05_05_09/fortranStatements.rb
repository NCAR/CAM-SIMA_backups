...
#$$$here...  why not just leave the "\n"s in to begin with and avoid all this crap?  
  ...
  def classify
    ...
    if (@statement =~ /^\s*\d*\s*end\s+module\s+(\w+)\s*$/i) then
      @module_end = true
      @module_name = $1.dup
    else
      @module_end = false
...
class FortranStatements
  ...
  # raise an exception if a module begin and end does not match.  
  ...
  def find_modules
    ...
    @statements.each_with_index do |statement, indx|
      ...
      if (statement.module_begin) then
        modules_begin_list << statement.module_name
        modules_begin[statement.module_name] = indx
      end
      if (statement.module_end) then
        modules_end_list << statement.module_name
        modules_end[statement.module_name] = indx
      end
    end
    missing_begins = modules_end_list - modules_begin_list
    unless (missing_begins.empty?) then
      missing_begins.each do |m|
        print "ERROR:  in file #{@fileName}, module #{m} has end but no beginning\n"
      end
      raise
    end
    missing_ends = modules_begin_list - modules_end_list
    unless (missing_ends.empty?) then
      missing_ends.each do |m|
        print "ERROR:  in file #{@fileName}, module #{m} has beginning but no end.  This module must end with \"END MODULE #{m}\"\n"
      end
    end
    modules_begin.each_key do |key|
      ret[key] = (modules_begin[key]..modules_end[key])
    end
    ret
  end
