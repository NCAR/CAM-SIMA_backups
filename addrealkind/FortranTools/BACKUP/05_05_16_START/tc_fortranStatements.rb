...
class TestFortranStatements
  ...
  def findsymbols_correct?
    ret = true
    if (@symbol) then
      symbols = @statements.findSymbols(@symbol)
  ...
  def findsymbolspublic_correct?
    ret = true
    if (@symbol) then
      symbols = @statements.findSymbols(@symbol, true)
      symbols.collect! { |sym| sym.upcase }
...
class TC_FortranStatements < Test::Unit::TestCase
  ...
  def setup
    ...
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/c_fpuse_OK.f90", nil, nil, nil) 
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/tst0_fpuse_OK.F90", nil, nil, nil) 
    @statementss << TestFortranStatements.new(
                      ...
                      "fake", 
                      ["FAKE1", "FAKE2", "FAKE21", "FAKEO", "FAKE_NOT_IN_MODULE"],
                      ["FAKE1"] )
    @statementss << TestFortranStatements.new(
    ...
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/dadadj_fpuse_OK.F90", nil, nil, nil) 
    @statementss << TestFortranStatements.new(
                      ...
                      "continuation_test/dadadjnor8_fpuse_OK.F90", nil, nil, nil) 
  end
